...
namespace ng
{
	...
	private:
		...
		struct node_t
		{
			...
			void draw_foreground (theme_ptr const& theme, ng::context_t const& context, bool isFlipped, bool leadingTab, CGRect visibleRect, ng::invisibles_t const& invisibles, ng::buffer_t const& buffer, size_t bufferOffset, std::vector< std::pair<size_t, size_t> > const& misspelled, CGPoint anchor, CGFloat baseline) const;
